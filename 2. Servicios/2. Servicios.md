...
<br />
<p align=center>
    <img src="https://github.com/hassanhabib/The-Standard-Spanish/blob/main/2.%20Agentes/Recursos/Agentes-1.0.0.png?raw=true" />
</p>
<br />
...
Los servicios pueden ser de diferentes tipos en función de su disposición en architectura tratada. Todos pueden ser clasificados en una de estas 3 categorías: validadores, orquestadores y agregadores.
...
Servicios de validación son principalmente servicios vecinos a los agentes o a los servicios fundamentales.
...
Los servicios de orquestación principalmente se centran en la combinación de múltiples operaciones primitivas o múltiples lógicas de negocio de orden superior para conseguir un objetivo más superior.
...
###### Este patrón sigue la simetría de el Patrón Florance
...
Podriamos devolver una agreación de la misma entidad ya sea un modelo personalizado o nativo como son List<Estudiante> o EstudiantesAgregados, o un tipo primitivo para obtener la cuenta de los estudiantes, o un boolean indicando ya sea si un estudiante existe o no pero no un contrato que no sea ni primitivo ni no agregado.
...
private async ValueTask<TarjetaBiblioteca> AsignaTarjetaBibliotecaAEstudianteAsync(Estudiante estudiante)
{
    ...
    return await this.servicioProcesamientoTarjetaBiblioteca.AddLibraryCardAsync(tarjetaBibliotecaEstudiante);
}
...
Como vimos arriba, un id de estudiante válido es requerido para garantizar una mapeo satisfactorio a TarjetaBiblioteca - y así como el mapeado es una responsabilidad del orquestador, requerimos garantizar que el estudiante de entrada y su id sean correctos antes de proceder con el proceso de orquestación.
